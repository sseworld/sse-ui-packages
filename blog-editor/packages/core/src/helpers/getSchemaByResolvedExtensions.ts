...
import { MarkSpec, NodeSpec, Schema } from "@sseui/pm/model";
...
export function getSchemaByResolvedExtensions(extensions: Extensions, editor?: Editor): Schema {
  ...
  const nodes = Object.fromEntries(
    nodeExtensions.map((extension) => {
      ...
      // if (parseHTML) {
      //   schema.parseDOM = parseHTML.map((parseRule) => {
      //     const modifiedRule = injectExtensionAttributesToParseRule(parseRule, extensionAttributes);
      //     modifiedRule.tag = typeof modifiedRule.tag === "string" ? modifiedRule.tag : "";
      //     return { ...modifiedRule } as const;
      //   });
      // }
